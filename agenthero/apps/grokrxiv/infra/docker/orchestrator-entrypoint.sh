...
copy_auth_item ".gemini/antigravity-cli"
copy_auth_item ".gemini/antigravity"
...
if [ "${GROKRXIV_REQUIRE_CLI_AUTH:-0}" = "1" ]; then
  ...
  [ -d "${AUTH_HOME}/.gemini/antigravity-cli" ] || [ -d "${AUTH_HOME}/.gemini/antigravity" ] || missing="${missing} antigravity"
  if [ -n "$missing" ]; then
