...
load_dotenv() {
  ...
  source "${env_file}"
  local env_files="${AGENTHERO_ENV_FILES:-}"
  local env_path
  ...
  for env_path in "${split_env_files[@]}"; do
    ...
    if [[ ! -f "${env_path}" ]]; then
      printf 'AGENTHERO_ENV_FILES references missing file %s\n' "${env_path}" >&2
      exit 1
    fi
    # shellcheck disable=SC1090
    source "${env_path}"
...
"${agenthero_cmd[@]}" --status --json app run grokrxiv ingest "${ARXIV_ID}" --runner "${GROKRXIV_RUNNER}" --extractor "${GROKRXIV_EXTRACTOR}" --no-cache --json 2>&1 \
  | tee /tmp/grokrxiv-publish-ingest.log
review_id="$(extract_json_field /tmp/grokrxiv-publish-ingest.log '.output.stdout | fromjson | if type == "array" then .[0].review_id else .review_id end' \
...
"${agenthero_cmd[@]}" --status --json app run grokrxiv approve "${review_id}" --json 2>&1 \
  | tee /tmp/grokrxiv-publish-approve.log
pr_url="$(extract_json_field /tmp/grokrxiv-publish-approve.log '.output.stdout | fromjson | .pr_url' \
