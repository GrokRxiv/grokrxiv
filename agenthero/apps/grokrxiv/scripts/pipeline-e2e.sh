...
bash "${APP_ROOT}/scripts/preflight.sh"
ok "preflight passed"

step "agent routing"
if [[ "${VALIDATE_PROVIDER_MODELS:-0}" == "1" ]]; then
  bash "${APP_ROOT}/scripts/validate-agent-routing.sh" --live
else
...
preview_status="$(curl -sS -o "${preview_body}" -w '%{http_code}' \
  -F "file=@${APP_ROOT}/tests/fixtures/sample.pdf;type=application/pdf" \
...
pipeline_log="$(mktemp)"
SKIP_APPROVE=1 bash "${APP_ROOT}/tests/m1-pipeline.sh" "${ARXIV_ID}" | tee "${pipeline_log}"
...
for name in summary technical_correctness novelty reproducibility citation meta_reviewer; do
  unzip -l ".agenthero/artifacts/grokrxiv/reviews/${review_id}/bundle.zip" "agents/${name}.json" >/dev/null \
...
if [[ "${RUN_PUBLISH_E2E:-0}" == "1" ]]; then
  step "PR handoff E2E"
  bash "${APP_ROOT}/scripts/publish-e2e.sh" "${ARXIV_ID}"
