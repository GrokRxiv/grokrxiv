...
#
# Usage:
#   agenthero/apps/grokrxiv/scripts/dev-webhook.sh up
#                                   Start orchestrator (if needed), start
#                                   cloudflared, upsert the webhook on
#                                   GrokRxiv/grokrxiv-reviews. Idempotent.
#   agenthero/apps/grokrxiv/scripts/dev-webhook.sh down
#                                   Kill cloudflared, delete the webhook.
#   agenthero/apps/grokrxiv/scripts/dev-webhook.sh status
#                                   Show current orchestrator / tunnel / hook state.
#   agenthero/apps/grokrxiv/scripts/dev-webhook.sh logs
...
#   - app-local docker compose has the `orchestrator` service defined
...
env_files() {
  ...
  printf '%s\n' "$ENV_FILE"
  local includes
  includes=$(grep -E '^AGENTHERO_ENV_FILES=' "$ENV_FILE" | head -1 | cut -d= -f2- || true)
  local env_path
...
env_file_for_key() {
  ...
  if [[ -f "$APP_ROOT/env/.env_publish" ]]; then
    printf '%s\n' "$APP_ROOT/env/.env_publish"
  else
    printf '%s\n' "$ENV_FILE"
  fi
}
...
ensure_orchestrator() {
  ...
  (cd "$ROOT" && docker compose -f "$COMPOSE_FILE" up -d orchestrator >/dev/null)
...
restart_orchestrator_if_env_changed() {
  ...
  if (( need_restart )); then
    (cd "$ROOT" && docker compose -f "$COMPOSE_FILE" up -d --force-recreate orchestrator >/dev/null)
    for _ in $(seq 1 30); do
...
cmd_down() {
  ...
  rm -f "$URL_FILE"
  log "tunnel + hook torn down. orchestrator left running (use docker compose -f $COMPOSE_FILE stop orchestrator)"
}
