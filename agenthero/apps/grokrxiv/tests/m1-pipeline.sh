...
# `agh app run grokrxiv review ... --json` invocation runs the full ingest+DAG+verify
...
#
# Requires:  agh, jq, an active DATABASE_URL, and the
...
#
# Usage:     agenthero/apps/grokrxiv/tests/m1-pipeline.sh [arxiv_id]
#
# Env knobs:
...
if ! command -v "${AGENTHERO_CMD[0]}" >/dev/null 2>&1; then
  echo "fatal: ${AGENTHERO_CMD[0]} not on PATH — install agh or set AGENTHERO_BIN" >&2
  exit 2
fi

if [[ "${RUNNER}" == "api" || "${EXTRACTOR}" == "api" ]]; then
  export AGENTHERO_ALLOW_PROVIDER_API=1
  "${AGENTHERO_CMD[@]}" doctor --json \
...
# Ingest + review.
response="$("${AGENTHERO_CMD[@]}" --status --json app run grokrxiv review "$ARXIV_ID" --runner "${RUNNER}" --extractor "${EXTRACTOR}" --no-cache --json)"
out="$(echo "$response" | jq -c '.output.stdout | fromjson')"
...
appr_response="$("${AGENTHERO_CMD[@]}" --json app run grokrxiv approve "$rid" --json)"
appr="$(echo "$appr_response" | jq -c '.output.stdout | fromjson')"
