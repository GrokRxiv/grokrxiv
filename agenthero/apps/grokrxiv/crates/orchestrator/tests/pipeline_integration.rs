...
async fn write_body_md_to_workdir_puts_body_where_tools_read_it() -> anyhow::Result<()> {
    ...
    grokrxiv_app_runtime::ingest_pipeline::write_body_md_to_workdir(work.path(), body).await?;
...
async fn load_paper_extract_resolves_section_bodies() -> anyhow::Result<()> {
    ...
    let extract = grokrxiv_app_runtime::ingest_pipeline::load_paper_extract(&repo_path, &ri)?;
