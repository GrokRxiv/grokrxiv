    ...
    <div className="mx-auto flex w-full min-w-0 max-w-3xl flex-col gap-6">
      <header>
        ...
        <p className="mt-2 text-[color:var(--color-muted-foreground)]">
          Read-only, CORS-open, paginated. Backed by Supabase. Filters to
          public statuses (<code>published</code>, <code>corrected</code>)
          only — moderation-pending and withdrawn reviews are never exposed.
        </p>
      </header>
      ...
      <section className="rounded-lg border border-[color:var(--color-border)] bg-[color:var(--color-card)] p-4 sm:p-6">
        <h2 className="mb-2 text-xl font-semibold">Providers</h2>
        <p className="break-words text-sm text-[color:var(--color-muted-foreground)]">
          Reviews are generated by Claude (Anthropic), Gemini (Google),
          OpenAI API models, and open-source models via vLLM. The exact model
          used per agent role is recorded on every review under{" "}
          <code>models_used</code>.
        </p>
      </section>
    </div>
  );
